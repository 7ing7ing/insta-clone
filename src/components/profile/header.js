...
export default function Header({
  ...
  useEffect(() => {
    const isLoggedInUserFollowingProfile = async () => {
      ...
      setIsFollowingProfile(!!isFollowing); //!! = double bang. Converts Object to boolean. If it was falsey (e.g. 0, null, undefined, etc.), it will be false, otherwise, true.
    };
    ...
    <div className="grid grid-cols-3 gap-4 justify-between mx-auto max-w-screen-lg">
      <div className="container flex justify-center items-center">
        {profileUsername ? (
          ...
          <img
            className="rounded-full h-16 w-16 md-h-20 lg:h-40 lg:w-40 flex"
            alt={`$Ting-Ting Chen's profile picture`}
            src="/images/avatars/ting.jpg"
          />
        )}
      </div>
      ...
      <div className="flex items-center justify-center flex-col col-span-2">
        <div className="container flex items-center">
          <p className="text-2-xl mr-4">{profileUsername}</p>
          {activeBtnFollow && (
            <button
